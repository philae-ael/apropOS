...
mem_block* mem_init_block(mem_block* block, size_t block_length, mem_block* prev){
    debugf_func("Adding block %X of length %X, prev is %X", block, block_length, prev);
...
void mem_block_split(mem_block* free_block, size_t size){
    debugf_func("free block %X, size header %d, size %d, maybe_next_free %X", free_block, sizeof(mem_block), size, &free_block->maybe_next_free);
    ...
    debugf_func("next block %X, size header %d, size %d, maybe_next_free %X", next_block, sizeof(mem_block), size, &next_block->maybe_next_free);
}
